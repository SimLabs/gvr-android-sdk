...
class StreamDecoder(
    ...
    private val decoder: MediaCodec = try {
        val codec = MediaCodec.createDecoderByType(VIDEO_FORMAT)
        Log.i("MediaCodecInfo", "${codec.codecInfo.getCapabilitiesForType(codec.codecInfo.supportedTypes[0]).colorFormats.joinToString(" ")} ")
        codec
    } catch (e: Throwable) {
